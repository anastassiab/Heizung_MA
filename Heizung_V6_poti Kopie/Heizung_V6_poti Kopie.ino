...
int pin_Poti = 34; // Anaqlog pin für Poti
int potValue = 0;
//float voltage = 0;
...
void setup() {
  ...
  pinMode(pin_Poti, INPUT);//_______pin Poti als Input definiert

  //PID Setup

  //SetpointTemperatur1 = 40;
...
void loop() {
  ...
  int potValue = analogRead(pin_Poti);
 // float voltage = floatMap(potValue, 0, 4095, 0, 3.3);
 double SetpointTemperatur1 = doubleMap(double(potValue), 0, 4095, 20, 50);
...
// Ausgabe der Daten von Heizelement 1
  ...
  double SetTemp1= SetpointTemperatur1;
...
// Ausgabe der Daten von Heizelement 2    
  ...
  double SetTemp2= SetpointTemperatur2; 
  
  Serial.print(SetTemp2);
  Serial.print("\t");
